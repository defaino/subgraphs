...
export function onBorrowed(event: Borrowed): void {
    ...
    user.borowed = user.borowed.plus(event.params.); // replace with usd amount
...
export function onBorrowRepaid(event: BorrowRepaid): void {
    ...
    user.borowed = user.borowed.minus(event.params.); // replace with usd amount

    if (user.borowed.equals(BigInt.zero())){
